...
class HtmlPieces
{
    ...
    public function get(object $page, string $element, string $url='')
    {
        ...
        switch ($element) {
            ...
            case "seasons":
                $seasons = [];
                $findAllSeasons = $dom->find($page, '[data-testid="tab-season-entry"]');

                $dom = new \PHPHtmlParser\Dom();
                foreach ($findAllSeasons as $seasonRow){
                    $season = [];
                    $seasonValue = $seasonRow->text;
                    $season['season'] = $seasonValue;
                    // Using imdb ajax api to get episodes
                    $season['episodes'] = $this->get($dom->loadFromUrl($url."/?season=".$seasonValue), "episodes");
            ...
            case "episodes":
                ...
                foreach ($findAllEpisodes as $episodeRow){
                    ...
                    $image = $episodeRow->find(".ipc-image");
                    if($this->count($image)) {
